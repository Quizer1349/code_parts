...
class LocationManager: NSObject,CLLocationManagerDelegate {
    ...
    func getDistanceToCoordinates(latitude latitude: Double! , longitude: Double!) -> Float{
    
        var distance = 0.0
        
        let latitude:CLLocationDegrees = CLLocationDegrees(latitude)
        let longitude:CLLocationDegrees = CLLocationDegrees(longitude)
        
        let destinationLoaction:CLLocation = CLLocation(latitude: latitude, longitude: longitude)
        
        let myLatitude:CLLocationDegrees = CLLocationDegrees(lastKnownLatitude)
        let myLongitude:CLLocationDegrees = CLLocationDegrees(lastKnownLongitude)
        
        let myLocation:CLLocation = CLLocation(latitude: myLatitude, longitude: myLongitude)
        
        distance = destinationLoaction.distanceFromLocation(myLocation)
    ...
    private func reverseGocodeUsingGoogle(latitude latitude:Double, longitude: Double){
        ...
        var urlString = "http://maps.googleapis.com/maps/api/geocode/json?latlng=\(latitude),\(longitude)&language=en" as NSString
...
private class AddressParser: NSObject{
    ...
    private func getAddressDictionary()-> NSDictionary {
        ...
        addressDict.setValue(ISOcountryCode, forKey: "country")
        addressDict.setValue(formattedAddress, forKey: "formattedAddress")
        addressDict.setValue(administrativeArea, forKey: "state")
        addressDict.setValue(administrativeAreaCode, forKey: "shortState")
    ...
    private func getPlacemark() -> CLPlacemark {
        ...
        var addressDict:[String:AnyObject] = Dictionary()
